...
For his part, Dr. Hoare refers to his invention as his "billion dollar mistake". He does not mean this as hyperbole. He means that he believes that a billion dollars literally is a pretty good ballpark figure for the amount of damage that has resulted from error caused by null references in the past half a century.
...
Some languages, such as Kotlin and Ceylon, attempt to wrangle null references by adding special syntax to indicate what variables are "nullable". Other languages provide special types, such as Haskell's `Maybe` and Scala's `Option`, that act like containers of zero or one element. Java 8 takes the latter approach, providing the generic `Optional` class. For earlier versions of Java, the Guava library provides a similar class, also named `Optional`, which is not as powerful but still works well for keeping track of variables that may or may not have values.
